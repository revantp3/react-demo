...
const articlesSlice = createSlice({
  ...
  initialState,
  reducers: {
    sortArticles: (state, action: PayloadAction<'date_desc' | 'date_asc' | 'title_asc' | 'title_desc'>) => {
...
// Export actions
export const { sortArticles, setCategoryFilter, setAuthorFilter, setCurrentPage } = articlesSlice.actions;
...
export const selectFilteredArticles = createSelector(
  ...
  (articles, filters) => {
    return articles.filter(article => {
      const matchesCategory = filters.source ? article.source === filters.source : true;
